...
- "We're offboarding this device, stop tracking it as ours (but leave it in
  Fleet)" → `mspbotsfleet_release_host`
...
| `mspbotsfleet_list_host_scripts` | 列出该主机可执行的脚本 | `host_id`(必填) |
...
| `mspbotsfleet_claim_hosts` | 声明标识符并接管匹配到的无主主机（幂等，可重复调用重试未匹配的） | `identifiers`(可选，主机名/序列号/UUID 列表，最多500条；省略/空数组=只重试之前声明过的) |
| `mspbotsfleet_release_host` | 放弃本租户对某主机的归属，不删除主机本身（幂等） | `host_id`(必填) |

> 主机列表为全量拉取后在内存过滤/排序/分页，Fleet 侧数据缓存 15 秒，归属信息实时查库不进缓存。
> `claim_hosts` 返回 `{claimed, waiting, rejected}`：`waiting` 是已声明但主机还没出现在 Fleet 里，
> `rejected` 是标识符已被别的租户声明过。

### Scripts

| Tool | 功能 | 参数 |
|---|---|---|
| `mspbotsfleet_list_scripts` | 列出本租户脚本 | `page`、`per_page` |
| `mspbotsfleet_get_script` | 获取脚本详情（含正文） | `script_id`(必填) |
| `mspbotsfleet_create_script` | 新建脚本（文件名须以 `.sh`/`.ps1` 结尾）。**非幂等**——重复调用会建出多个脚本 | `name`(必填)、`contents`(必填，≤500KB) |
...
> `create_script`/`update_script` 只返回 `{id, name}`，时间戳等其余字段需再查
> `mspbotsfleet_get_script`/`mspbotsfleet_list_scripts`。

### Saved queries

| Tool | 功能 | 参数 |
|---|---|---|
| `mspbotsfleet_list_queries` | 列出本租户查询 | `page`、`per_page` |
| `mspbotsfleet_get_query` | 获取查询详情（含 SQL） | `query_id`(必填) |
| `mspbotsfleet_create_query` | 新建保存查询（仅允许单条 SELECT / WITH…SELECT）。**非幂等**——重复调用会建出多个查询 | `name`(必填)、`sql`(必填)、`description`、`platform`(`darwin`/`windows`/`linux`) |
